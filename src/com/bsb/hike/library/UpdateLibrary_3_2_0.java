...
public class UpdateLibrary_3_2_0 extends UpdateLibrary {
...
//				clickOnElement(Locators.CONTENT_DESCRIPTION , WelcomeScreen.HIKE_MESSENGER_LOGO_ICON);
	...
	public void addHikeContactAsFavorite(String version){
		try {
			...
			goToHome();
			openOverflowMenu();
			clickOnElement(Locators.NAME, "Favorites");
...
//			Assert.assertTrue(searchElementInListWithouTAsserting(Locators.NAME,"Added as a favorite"));
	...
	public void setStatusUpdate() throws UiObjectNotFoundException, InterruptedException {
		try {
			...
			UiObject View =getElement(Locators.CLASSNAME, AndroidClassNames.VIEW,0);
			UiObject LLayout=getChild(View, Locators.CLASSNAME, AndroidClassNames.LINEAR_LAYOUT, 1);
			UiObject timeline=getChild(LLayout, Locators.CLASSNAME, AndroidClassNames.RELATIVE_LAYOUT,1);

			clickOnElement(timeline);
	...
	public void toggleLastSeenCheckbox(){
		try {
			System.out.println("CHANGING LAST SEEN SETTINGS");

			goToHome();
			openOverflowMenu();
			clickOnElement(Locators.NAME, OverFlowListScreen.SETTINGS_LBL);
			clickOnElement(Locators.NAME , SettingsScreen.PRIVACY_LBL);
			UiObject object = new UiObject(new UiSelector().className("android.widget.CheckBox").instance(1));
			object.click();
			privacyCheckboxStatus.put(0, object.isChecked());
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
